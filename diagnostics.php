<?php 
    ...
    if (R::findOne('meshbotusers', 'tg_id = ?', array($userId))) {
        ...
        if (!isset(json_decode($user -> diags, true)['message']) && isset($user -> diags)) {
            $result1 = json_decode($user -> diags, true);
        }
        else {
        ...
        $message = "***📊 Результаты независимых диагностик***:\n\n";
        foreach ($result1['data'] as $f) {
            $message .= "***".$f['learningYear'].":***\n";
            foreach ($f['diagnostics']['diagnostic'] as $i) {
                $message .= "🟢 ".$i['name']." ___(".$i['subject'].")___"." - ***".$i['resultValue']."/".$i['maxResult']."*** ___(".$i['levelType']." уровень)___\n\n";
            }
        }
        $keyboard = [
            "inline_keyboard" => [
